...
\newcommand\ifriscv[1]{\if\architecture\riscvEd{#1}\fi}
\newcommand\ifxeightsix[1]{\if\architecture\xeightysixEd{#1}\fi}
\newenvironment{riscv}{\if\architecture\riscvEd}{\fi}
\newenvironment{xeightysix}{\if\architecture\xeightysixEd}{\fi}
...
The compiler targets {\ArchName} assembly language~\citep{Intel:2015aa}, so it
is helpful but not necessary for the reader to have taken a computer
systems course~\citep{Bryant:2010aa}. We introduce the parts of {\ArchName}-64
...
\racket{Racket}\python{Python} to {\ArchName}-64 assembly
code~\citep{Intel:2015aa}. The subset, named \LangVar{}, includes
integer arithmetic and local variables.  We often refer to {\ArchName}-64
simply as {\ArchName}.  The chapter first describes the \LangVar{} language
(section~\ref{sec:s0}) and then introduces {\ArchName} assembly
(section~\ref{sec:x86}). Because {\ArchName} assembly language is large, we
discuss only the instructions needed for compiling \LangVar{}. We
introduce more {\ArchName} instructions in subsequent chapters.  After
introducing \LangVar{} and {\ArchName}, we reflect on their differences and
create a plan to break down the translation from \LangVar{} to {\ArchName}
...
friends, can implement a compiler from \LangVar{} to {\ArchName} in a short
...
any program $P_1$ written in the \LangVar{} language into an {\ArchName} assembly
...
\end{tikzpicture}
\]
Next we introduce the \LangXInt{} subset of {\ArchName} that suffices for
compiling \LangVar{}.
...
\LangXInt{}.  We use the \ifxeightsix{AT\&T} syntax expected by the GNU
assembler.
%
...
%
An {\ArchName} program is stored in the computer's memory.  For our purposes,
the computer's memory is a mapping of 64-bit addresses to 64-bit
values.  The computer has a \emph{program counter}
(PC)\index{subject}{program counter}\index{subject}{PC}
stored in the \ifxeightsix{\code{rip}}\ifriscv{\code{pc}} register that points to the address of the next instruction
...
next instruction in memory. Most {\ArchName} instructions take \ifxeightsix{two}\ifriscv{three} operands,
each of which can be an integer constant (called an \emph{immediate
...
\newcommand{\allregisters}{\key{rsp} \MID \key{rbp} \MID \key{rax} \MID \key{rbx} \MID \key{rcx}
              ...
              \MID \key{r14} \MIDn \key{r15}}
\fi
\if\architecture\riscvEd
\newcommand{\allregisters}{\key{sp} \MID \key{fp} \MID \key{ra} \MID
  \key{zero} \MID \key{pc} \MID \\
  && \key{a0} - \key{a7} \MID \key{s2} - \key{s11} \MID \key{t0} - \key{t6}}
\fi
...
\Reg &::=& \allregisters{} \\
\Arg &::=&  \key{\$}\Int \MID \key{\%}\Reg \MID \Int\key{(}\key{\%}\Reg\key{)}\\
\Instr &::=& \key{add} \; \Arg\key{,} \Arg \MID
      \key{sub} \; \Arg\key{,} \Arg \MID
      \key{neg} \; \Arg \MID \key{mv} \; \Arg\key{,} \Arg \MID \\
...
% \Arg &::=&  \key{\$}\Int \MID \key{\%}\Reg \MID \Int\key{(}\key{\%}\Reg\key{)}\\
  \Instr &::=&
               \key{add} \; \Reg\key{,} \Reg\key{,} \Reg \MID
               \key{addi} \; \Reg\key{,} \Reg\key{,} \Int \MID
               \key{sub}  \; \Reg\key{,} \Reg\key{,} \Reg \MID \\ &&
               \key{neg}  \; \Reg\key{,} \Reg \MID
               \key{mv} \; \Reg\key{,} \Reg \MID
               \key{li} \; \Reg\key{,} \Int \MID
  \\
  &&  \key{call} \; \mathit{label} \MID
     \key{sd}\;\Reg\key{,}\Int\key{(}\Reg\key{)} \MID
     \key{ld}\;\Reg\key{,}\Int\key{(}\Reg\key{)} \MID \key{ret} \\
\LangXIntM{} &::= & \key{.globl main}\\
      &    & \key{main:} \; \Instr^{*}
\end{array}
\]
...
\fi}
\end{tcolorbox}
\caption{The syntax of the \LangXInt{} assembly language\ifxeightsix{ (AT\&T syntax)}.}
...
value. There are \ifxeightsix{16}\ifriscv{32} general-purpose registers in the computer; their
names are given in figure~\ref{fig:x86-int-concrete}. \ifriscv{They
  also have generic names \code{x0} - \code{x31}.}  A register is
\ifriscv{simply referenced by its name.}\ifxeightsix{
written with a percent sign, \key{\%}, followed by its name,
for example, \key{\%rax}.}

An immediate value is written using the notation \ifxeightsix{\key{\$}}$n$ where $n$
is an integer.
%
%
An access to memory is specified using the syntax \ifxeightsix{$n(\key{\%}r)$}\ifriscv{$n(r)$},
which obtains the address stored in register $r$ and then adds an
offset of $n$ bytes
to the address. The resulting address is used to load or to store
to memory depending on whether it occurs as a source or destination
argument of an instruction.

\if\architecture\xeightysixEd
  An arithmetic instruction such as $\key{addq}\,s\key{,}\,d$ reads
  from the source $s$ and destination $d$, applies the arithmetic
  operation, and then writes the result to the destination
  $d$.\index{subject}{instruction}
\fi
\if\architecture\riscvEd
  An arithmetic instruction such as $\key{add}\,d\key{,}s_1\key{,}s_2$ reads from
  the source registers $s_1$ and $s_2$, applies the arithmetic
  operation, and then writes the result to the destination register
  $d$.
  Some instructions come with an immediate version like
  $\key{addi}\,d\key{,}s\key{,}n$, which adds the number $n$ to the source
  register $s$ and writes the result to the destination register $d$.
\fi
%
\if\architecture\xeightysixEd
...
\fi
\if\architecture\riscvEd
  The move instruction $\key{mv}\,d\key{,}s$ reads from register $s$ and
  stores the result in register $d$.
\fi
%
The \ifxeightsix{$\key{callq}\,\itm{label}$}\ifriscv{$\key{call}\,\itm{label}$} instruction jumps to the procedure
specified by the \itm{label}, and \ifxeightsix{$\key{retq}$}\ifriscv{$\key{ret}$} returns from a procedure to
its caller.
%
...
all the {\ArchName} instructions used in this book.

Figure~\ref{fig:p0-x86} depicts an {\ArchName} program that computes
\racket{\code{(+ 10 32)}}\python{10 + 32}. The instruction
\if\architecture\xeightysixEd\lstinline{movq $10, %rax}
\fi
\ifriscv{\lstinline{li a0, 10}}
puts $10$ into register \ifxeightsix{\key{rax}}\ifriscv{\key{a0}}, and
then
\if\architecture\xeightysixEd\lstinline{addq $32, %rax}
\fi
\ifriscv{\lstinline{addi a0, a0, 32}}
adds $32$ to the $10$ in \ifxeightsix{\key{rax}}\ifriscv{\key{a0}} and
puts the result, $42$, into \ifxeightsix{\key{rax}}\ifriscv{\key{a0}}.
%
The last instruction \ifxeightsix{\key{retq}}\ifriscv{\key{ret}} finishes the \key{main} function by
returning the integer in \ifxeightsix{\key{rax}}\ifriscv{\key{a0}} to the operating system. The
...
\caption{An {\ArchName} program that computes
  \racket{\code{(+ 10 32)}}\python{10 + 32}.}
\label{fig:p0-x86}
\end{figure}

We exhibit the use of memory for storing intermediate results in the
next example.  Figure~\ref{fig:p1-x86} lists an {\ArchName} program that
...
shown in figure~\ref{fig:frame}.  The register
\ifxeightsix{\key{rsp}}\ifriscv{\key{sp}} is called the 
...
caller side.  The function call instruction,
\ifxeightsix{\code{callq}}\ifriscv{\code{call}}, pushes the 
return address onto the stack prior to jumping to the procedure.  The
register \ifxeightsix{\key{rbp}}\ifriscv{\key{fp}} is the \emph{\ifxeightsix{base}\ifriscv{frame} pointer}\index{subject}{base
  pointer} and is used to access variables that are stored in the
frame of the current procedure call.  The \ifxeightsix{base}\ifriscv{frame} pointer of the caller
...
stored at address
\ifxeightsix{$-8\key{(\%rbp)}$}\ifriscv{$-24(\key{fp})$},
variable $2$ at
\ifxeightsix{$-16\key{(\%rbp)}$}\ifriscv{$-32(\key{fp})$}, and so on.
...
\begin{tabular}{|r|l|} \hline
Position & Contents \\ \hline
$-8$(\key{fp}) & return address \\
$-16$(\key{fp}) & old \key{fp} \\
$-24$(\key{fp}) & variable $1$ \\
$-32$(\key{fp}) & variable $2$ \\
 \ldots  & \ldots \\
$0$(\key{sp}) & variable $n$\\ \hline
\end{tabular}
\fi
\end{tcolorbox}
\end{minipage}
...
The operating system issues a \ifxeightsix{\code{callq
    main}}\ifriscv{\code{call main}} instruction that
\ifxeightsix{pushes its return address on the stack}
\ifriscv{stores the return address in register \key{ra}} and then jumps to
\code{main}. In {\ArchName}, the stack pointer \ifxeightsix{\code{rsp}}\ifriscv{\code{sp}} must be divisible
by 16 bytes prior to the execution of any \ifxeightsix{\code{callq}}\ifriscv{\code{call}} instruction.
\if\architecture\xeightysixEd
That is when control arrives at \code{main}, the \code{rsp} is 8 bytes
out of alignment (because the \code{callq} pushed the return address).
\fi
The first \ifxeightsix{three}\ifriscv{four} instructions are the typical
\emph{prelude}\index{subject}{prelude} for a procedure.
\if\architecture\xeightysixEd
The instruction \code{pushq \%rbp} first subtracts $8$ from the stack
...
make enough room for storing variables.
This program needs one
...
\fi
\if\architecture\riscvEd
The instruction \code{addi sp, sp,-32} first subtracts $32$ from the stack
pointer \code{sp} to make room for a stack frame that can
accommodate two variables. The next instruction \code{sd ra, 24(sp)}
saves the return address at the top of the stack frame. The subsequent
instruction \code{sd fp, 16(sp)} saves the previous frame pointer
right below. The final instruction 
\code{addi fp, sp, 32} sets the frame pointer on top of the return
address. This place is advantageous as it makes \key{fp} point to the
arguments of the caller, as we will see.
\fi
...
\fi
\if\architecture\riscvEd
\code{li a0, 52} which loads $52$ into register \key{a0}.
The next instruction \code{li a1, 10} load $10$ into register
\key{a1}.
Finally, \code{sub a0, a0, a1} replaces the value in \key{a0} with
$52-10=42$.
The caller of the function expects the return value in \key{a0}, so
...
\python{The \emph{conclusion}\index{subject}{conclusion} of the
  \code{main} function consists of the last \ifxeightsix{three}\ifriscv{four} instructions.}
%
\if\architecture\xeightysixEd
...
\fi
\if\architecture\riscvEd
The first two instruction restore the return address to  \key{ra} and
the saved frame pointer to \key{fp}. The instruction \code{add sp, sp,
32} restores the stack pointer to its value on entry to the
function. The last instruction \code{ret} jumps back to the caller.
\fi

Our compiler needs a convenient representation for manipulating {\ArchName}
programs, so we define an abstract syntax for {\ArchName}, shown in
...
\fi} %
{\if\edition\racketEd  
The main difference between this language and the concrete syntax of \LangXInt{}
...
label associated with every basic block; this is why the \key{{\ARCHNAME}Program}
...
\fi}
%
Regarding the abstract syntax for
\ifxeightsix{\code{callq}}\ifriscv{\code{call}}, the
\ifxeightsix{\code{Callq}}\ifriscv{\code{Call}} AST 
...
\fi
\if\architecture\riscvEd
\newcommand{\allastregisters}{\skey{sp} \MID \skey{fp} \MID \skey{ra} \MID \skey{zero} \MID \skey{pc}
              \MID \skey{a0} - \skey{a7} \MID \skey{s1} \MID \\
              && \skey{s2} - \skey{s11} \MID \skey{t0} - \skey{t6}}
\fi
...
\Arg &::=&  \IMM{\Int} \MID \REG{\Reg}
   \MID \DEREF{\Reg}{\Int} \\
\Instr &::=& \TERINSTR{\skey{add}}{\Arg}{\Arg}{\Arg} 
       \MID \TERINSTR{\skey{sub}}{\Arg}{\Arg}{\Arg}\\
       &\MID& \BININSTR{\skey{neg}}{\Arg}{\Arg}
       \MID \BININSTR{\skey{mv}}{\Arg}{\Arg}\\
      &\MID& \CALLQ{\itm{label}}{\itm{int}}
       \MID \RETQ{}
       \MID \JMP{\itm{label}}  \\
\Block &::= & \Instr^{+} 
...
\Arg &::=&  \IMM{\Int} \MID \REG{\Reg}
   \MID \DEREF{\Reg}{\Int} \\
  \Instr &::=& \TERINSTR{\scode{add}}{\Arg}{\Arg}{\Arg}
               \MID \TERINSTR{\scode{sub}}{\Arg}{\Arg}{\Arg} \\
     &\MID&
            \BININSTR{\scode{neg}}{\Arg}{\Arg} 
            \MID \BININSTR{\scode{mv}}{\Arg}{\Arg} \\
%       &\MID& \BININSTR{\scode{sd}}{\Reg}{\DEREF{\Reg}{\Int}} \MID \BININSTR{\scode{ld}}{\Reg}{\DEREF{\Reg}{\Int}} \\
     &\MID& \CALLQ{\itm{label}}{\itm{int}} \MID \RETQ{}
            % \MID \JMP{\itm{label}}
  \\
\LangXIntM{} &::= & \XPROGRAM{}{\Instr^{*}}{}
% \TERINSTR{\scode{addi}}{\Reg}{\Reg}{\Int}
% \MID \BININSTR{\scode{li}}{\Reg}{\Int} \\
\end{array}
\]
\fi}
\end{tcolorbox}
...
\section{Planning the Trip to {\ArchName}}
...
and {\ArchName} assembly? Here are some of the most important ones:

\begin{enumerate}
\item
\if\architecture\xeightysixEd
  {\ArchName} arithmetic instructions typically have two arguments and
  update the second argument in place. In contrast, \LangVar{}
  arithmetic operations take two arguments and produce a new value.
  An {\ArchName} instruction may have at most one memory-accessing argument.
  Furthermore, some {\ArchName} instructions place special restrictions on
  their arguments.
\fi
\if\architecture\riscvEd
  {\ArchName} arithmetic instructions have three register arguments,
  the first one being the destination register. In contrast, \LangVar{}
  arithmetic operations take two arguments and produce a new value.
  An {\ArchName} arithmetic instruction cannot have a memory-accessing
  argument. Such an argument has to be loaded into a register, first.
\fi

\item An argument of an \LangVar{} operator can be a deeply nested
  expression, whereas {\ArchName} instructions restrict their arguments to be
  integer constants, registers, and memory locations.

{\if\edition\racketEd      
\item The order of execution in {\ArchName} is explicit in the syntax, which
...
\item A program in \LangVar{} can have any number of variables,
  whereas {\ArchName} has \ifxeightsix{16}\ifriscv{32} registers and the procedure call stack.
{\if\edition\racketEd    
\item Variables in \LangVar{} can shadow other variables with the
  same name. In {\ArchName}, registers have unique names, and memory locations
  have unique addresses.
\fi}  
\end{enumerate}

We ease the challenge of compiling from \LangVar{} to {\ArchName} by breaking
...
\item[\key{select\_instructions}]\index{subject}{select instructions}
  ...
  \LangVar{} operations and {\ArchName} instructions. This pass converts each
...
intertwined.
%
In chapter~\ref{ch:Lfun} we learn that in {\ArchName}, registers are used for
...
dead end in \key{assign\_homes}.
\if\architecture\xeightysixEd
Recall that only one argument of an
{\ArchName} instruction may be a memory access, but \key{assign\_homes} might
be forced to assign both arguments to memory locations.
\fi
\if\architecture\riscvEd
Recall that no argument of an
{\ArchName} arithmetic instruction may be a memory access, but \key{assign\_homes} might
be forced to assign an argument to a memory location.
\fi
%
...
language of this pass, \LangXVar{}, is a variant of {\ArchName} that still
...
translations of $\Atm_1$ and $\Atm_2$, respectively.)
\if\architecture\xeightysixEd
There is an
\key{addq} instruction in {\ArchName}, but it performs an in-place update.
%
So, we could move $\Arg_1$ into the \code{rax} register, then add
$\Arg_2$ to \code{rax}, and then finally move \code{rax} into
\itm{var}.
\fi
\if\architecture\riscvEd
There is a suitable \key{add} instruction in {\ArchName}, but it is
unable to deal with all kinds of atomic expressions.
%
We leave it like that for the moment and fix the arguments later.
\fi
\begin{transformation}
{\if\edition\racketEd    
\begin{lstlisting}
...
The \READOP{} operation does not have a direct counterpart in {\ArchName}
...
generated {\ArchName} assembly code, you need to compile \code{runtime.c} to
...
move from \ifxeightsix{\code{rax}}\ifriscv{\code{a0}} to the left-hand side variable.  (The
return value of a function is placed in \ifxeightsix{\code{rax}}\ifriscv{\code{a0}}.)  
\begin{transformation}
{\if\edition\racketEd  
\begin{lstlisting}
|$\itm{var}$| = (read);
...
In {\ArchName}, the first \ifxeightsix{six}\ifriscv{eight} arguments to functions are passed in registers,
with the first argument passed in register \ifxeightsix{\code{rdi}}\ifriscv{\code{a0}}. So we move the
$\Arg$ into \ifxeightsix{\code{rdi}}\ifriscv{\code{a0}} and then call \code{print\_int} using the
\ifxeightsix{\code{callq}}\ifriscv{\code{call}} instruction.
\begin{transformation}
\begin{lstlisting}
print(|$\Atm$|)    
\end{lstlisting}
...
\ifxeightsix{sixteen}\ifriscv{thirty two} registers, some programs must necessarily resort to placing
...
\ifxeightsix{\code{-8(\%rbp)}}\ifriscv{\code{-24(fp)}} and variable
\code{b} to location
\ifxeightsix{\code{-16(\%rbp)}}\ifriscv{\code{-32(fp)}}. 
\if\architecture\xeightysixEd
\begin{transformation}
...
\racket{
  ...
  \code{{\ARCHNAME}Program} node. The \code{locals-types} entry is an alist
  ...
  which you should propagate to the \code{{\ARCHNAME}Program} node.}
%
...
bytes) in
\racket{the $\itm{info}$ field of the \key{{\ARCHNAME}Program} node, with the key \code{stack-space},}
%
\python{the field \code{stack\_space} of the \key{{\ARCHNAME}Program} node,}
%
which is needed later to generate the conclusion of the \code{main}
procedure. The {\ArchName} standard requires the frame size to be a
...
restriction that all arguments of an arithmetic instruction must be registers. 
...
\end{minipage}\\
\if\architecture\riscvEd
None of the \code{mv} instructions is acceptable as generated, but two
of them can be implemented by load or store instructions.
...
\begin{lstlisting}
    ...
    call print_int
\end{lstlisting}
All load instructions in this program are redundant, but we leave their
elimination to later passes.

When patching the \code{add} instruction, there are two cases
to consider. First, one of the arguments may be immediate, in which
case we should generate a \code{addi} instruction, unless the
...
The last step of the compiler from \LangVar{} to {\ArchName} is to generate
...
compiler by executing the generated {\ArchName} code. It translates the {\ArchName}
...
translates the {\ArchName} AST that you produce into a string by invoking the
\code{repr} method that is implemented by the {\ArchName} AST classes in
\code{\ArchName\_ast.py}.
%
\fi}
...
In chapter~\ref{ch:Lvar} we learned how to compile \LangVar{} to {\ArchName},
...
completely in the {\ArchName} assembly language, but it still uses variables.
...
govern how function calls are performed in {\ArchName}.
%
...
compilers. Here we follow the \ifxeightsix{System V} calling conventions that are
used by the GNU C compiler\ifxeightsix{ on Linux and MacOS~\citep{Bryant:2005aa,Matz:2013aa}}.
%
...
In {\ArchName}, registers are also used for passing arguments to a function
and for the return value.  In particular, the first \ifxeightsix{six}\ifriscv{eight} arguments of a
function are passed in the following \ifxeightsix{six}\ifriscv{eight} registers, in this order.
\if\architecture\xeightysixEd
\begin{lstlisting}
...
\end{lstlisting}
\fi
We refer to these \ifxeightsix{six}\ifriscv{eight} registers as the \emph{argument-passing registers}
\index{subject}{argument-passing registers}.
If there are more than \ifxeightsix{six}\ifriscv{eight} arguments, the convention is to use space
on the frame of the caller for the rest of the arguments. In
chapter~\ref{ch:Lfun}, we instead pass a tuple containing the \ifxeightsix{sixth}\ifriscv{eighth}
...
%
The register\ifxeightsix{ \code{rax} is}\ifriscv{s \code{a0} and
  \code{a1} are} used for the return value of a function.
...
generated {\ArchName} code on the right-hand side. Variable \code{x} is
assigned to \ifxeightsix{\code{rbx}}\ifriscv{\code{s2}}, a callee-saved register. Thus, it is already
in a safe place during the second call to \code{read\_int}. Next,
variable \code{y} is assigned to \ifxeightsix{\code{rcx}}\ifriscv{\code{t0}}, a caller-saved register,
...
with saving the \ifxeightsix{\code{rbp}}\ifriscv{\code{fp}} register to the stack and setting 
\ifxeightsix{\code{rbp} to the current stack
  pointer}\ifriscv{\code{fp} to the call-time stack pointer}. We now know why it is
necessary to save the \ifxeightsix{\code{rbp}}\ifriscv{\code{fp}}: it
is a callee-saved register.
The prelude then pushes \ifxeightsix{\code{rbx}}\ifriscv{\code{s2}} to
the stack because (1) \ifxeightsix{\code{rbx}}\ifriscv{\code{s2}} is 
a callee-saved register and (2) \ifxeightsix{\code{rbx}}\ifriscv{\code{s2}} is assigned to a variable
(\code{x}). The other callee-saved registers are not saved in the
prelude because they are not used. The prelude subtracts another 8 bytes from
the \ifxeightsix{\code{rsp}}\ifriscv{\code{sp}} to make it 16-byte aligned. Shifting attention to the
conclusion, we see that \ifxeightsix{\code{rbx}}\ifxeightsix{\code{s2}} is restored from the stack with a
\ifxeightsix{\code{popq}}\ifriscv{\code{sd}} instruction.
\index{subject}{prelude}\index{subject}{conclusion}
...
$L_{\mathsf{after}}$ for the \ifxeightsix{\code{addq b,
    c}}\ifriscv{\code{add b, b, c}} instruction is
...
\]
The live-before for instruction \ifxeightsix{\code{movq a,
    c}}\ifriscv{\code{mv c, a}}
is $\{\ttm{a}\}$ because it writes to $\{\ttm{c}\}$ and reads from $\{\ttm{a}\}$
(formula~\eqref{eq:live-before-after-minus-writes-plus-reads}).  The
live-before for \ifxeightsix{\code{movq \$30, b}}\ifriscv{\code{mv b, 30}} is $\{\ttm{a}\}$ because it writes to a
variable that is not live and does not read from a variable.
Finally, the live-before for \ifxeightsix{\code{movq \$5,
    a}}\ifriscv{\code{mv a, 5}} is $\emptyset$
...
an instruction (the $W$ function). The \ifxeightsix{\code{callq}}\ifriscv{\code{call}} instruction should
...
during the function call. Likewise, the \ifxeightsix{\code{callq}}\ifriscv{\code{call}} instruction
should include the appropriate argument-passing registers in its
read set $R$, depending on the arity of the function being
called. (This is why the abstract syntax for \ifxeightsix{\code{callq}}\ifriscv{\code{call}} includes the
arity.)
\end{exercise}
...
A more economic way to compute the interference graph focuses on
writes~\citep{Appel:2003fk}. The writes performed by an instruction
must not overwrite a live location. So for each
...
itself.) For the \ifxeightsix{\key{callq}}\ifriscv{\key{call}}
instruction, we assume that the callee writes to all 
caller-saved registers, so an edge is added
between every live variable and every caller-saved register. There is
also a special treatment for the
\ifxeightsix{\key{movq}}\ifriscv{\key{mv}} instruction. If a live variable $v$ is the same as the source of the
move, then there is no need to add an edge between $v$ and the
destination. It is fine if they are mapped to the same register,
...
\item If instruction $I_k$ is a move instruction of the form
  \ifxeightsix{\key{movq} $s$\key{,} $d$}%
  \ifriscv{\key{mv} $d$\key{,} $s$}, then for every $v \in
  L_{\mathsf{after}}(k) \setminus \{ d, s\}$, add the edge
  $\{d,v\}$.

\item For any other instruction $I_k$, for every $d \in W(k)$ and
  every $v \in L_{\mathsf{after}}(k) \setminus \{d\}$, add the edge
  $\{d,v\}$.
\end{enumerate}
...
instructions.
\racket{The first instruction is \lstinline{movq $1, v},
  and the live-after set is $\{\ttm{v},\ttm{rsp}\}$. Rule 1 applies,
  so \code{v} interferes with \code{rsp}.}
%
\python{The first instruction is
  \ifxeightsix{\lstinline{movq $1, v}}\ifriscv{\lstinline{mv v, $1}}, and the
...
%
\python{The \ifxeightsix{fourth}\ifriscv{third} instruction is
  \ifxeightsix{\lstinline{addq $7, x}}\ifriscv{\lstinline{add x, v, $7}}, and the
...
%
\python{The next instruction is
  \ifxeightsix{\lstinline{movq x, y}}\ifriscv{\lstinline{mv y, x}}, and the
...
\if\architecture\riscvEd
\begin{tabular}{ll}
\lstinline!mv v, $1!& no interference\\
\lstinline!mv w, $42!& \ttm{w} interferes with \ttm{v}\\
\lstinline!add x, v, $7!& \ttm{x} interferes with \ttm{w}\\
\lstinline!mv y, x!& \ttm{y} interferes with \ttm{w} but not \ttm{x} or \ttm{y}\\
\lstinline!add z, x, w!& \ttm{z} interferes with \ttm{y} \\
\lstinline!mv tmp_0, y!& \ttm{tmp\_0} interferes with \ttm{z} \\
\lstinline!neg tmp_1, tmp_0!& \ttm{tmp\_1} interferes with \ttm{z} \\
\lstinline!add tmp_2, z, tmp_1! & no interference\\
\lstinline!mv a0, tmp_2! & no interference \\
\lstinline!call print_int!& no interference.
\end{tabular}
\fi
\fi}
\end{quote}
...
Register allocation is easier than sudoku, because the
register allocator can fall back to assigning variables to stack
locations when the registers run out. Thus, we replace
...
register allocation. In the example, we employ the following
correspondence, with \ifxeightsix{$k=11$}\ifriscv{$k=24$}.
\if\architecture\xeightysixEd
\begin{lstlisting}
...
to indicate that it has not yet been assigned a color.  Each register
node (not shown) should be assigned the number that the register
corresponds to, for example, color \code{t0} with the number \code{0}
and \code{t1} with \code{1}. The saturation sets are also shown for
each node; all of them start as the empty set. 
%
\[
\begin{tikzpicture}[baseline=(current  bounding  box.center),scale=0.9]
...
are all equally saturated. So we flip a coin and pick $\ttm{tmp\_0}$
and then we color it with the first available integer, which is $0$. We mark
$0$ as no longer available for $\ttm{z}$ because
it interferes with $\ttm{tmp\_0}$.
\[
\begin{tikzpicture}[baseline=(current  bounding  box.center),scale=0.9]
\node (t0) at (0,2) {$\ttm{tmp\_0}: 0, \{\}$};
\node (t1) at (0,0) {$\ttm{tmp\_1}: -, \{\}$};
...
\end{tikzpicture}
\]
We repeat the process. The most saturated vertex is \code{z}, so we choose \code{z} and color it with the first
available number, which is $1$. We add $1$ to the saturation for the
neighboring vertices \code{tmp\_0}, \code{tmp\_1}, and \code{y}.
\[
\begin{tikzpicture}[baseline=(current  bounding  box.center),scale=0.9]
\node (t0) at (0,2) {$\ttm{tmp\_0}: 0, \{1\}$};
\node (t1) at (0,0) {$\ttm{tmp\_1}: -, \{1\}$};
\node (t2) at (9,2) {$\ttm{tmp\_2}: -, \{\}$};
\node (z) at (3,2)  {$\ttm{z}: 1, \{0\}$};
\node (x) at (6,2)  {$\ttm{x}: -, \{\}$};
\node (y) at (3,0)  {$\ttm{y}: -, \{1\}$};
...
\end{tikzpicture}
\]
The most saturated vertices are now \code{tmp\_1} and
\code{y}. We color \code{y} with the first available color, which
is $0$.
\[
\begin{tikzpicture}[baseline=(current  bounding  box.center),scale=0.9]
\node (t0) at (0,2) {$\ttm{tmp\_0}: 0, \{1\}$};
...
\end{tikzpicture}
\]
Now the most saturated vertices are \code{w} and \code{tmp\_1}. We
color \code{w} with $1$.
\[
\begin{tikzpicture}[baseline=(current  bounding  box.center),scale=0.9]
\node (t0) at (0,2) {$\ttm{tmp\_0}: 0, \{1\}$};
...
\]
The most saturated vertices are \code{tmp\_1}, \code{x}, and \code{v}.
We choose to color \code{v} with $0$.
\[
\begin{tikzpicture}[baseline=(current  bounding  box.center),scale=0.9]
\node (t0) at (0,2) {$\ttm{tmp\_0}: 0, \{1\}$};
\node (t1) at (0,0) {$\ttm{tmp\_1}: -, \{1\}$};
...
\end{tikzpicture}
\]
We color the remaining three variables, \code{tmp\_1}, \code{tmp\_2}, and \code{x}, with $0$.
\[
\begin{tikzpicture}[baseline=(current  bounding  box.center),scale=0.9]
\node (t0) at (0,2) {$\ttm{tmp\_0}: 0, \{1\}$};
\node (t1) at (0,0) {$\ttm{tmp\_1}: 0, \{1\}$};
\node (t2) at (9,2) {$\ttm{tmp\_2}: 0, \{\}$};
\node (z) at (3,2)  {$\ttm{z}: 1, \{0\}$};
\node (x) at (6,2)  {$\ttm{x}: 0, \{1\}$};
\node (y) at (3,0)  {$\ttm{y}: 0, \{1\}$};
\node (w) at (6,0)  {$\ttm{w}: 1, \{0\}$};
\node (v) at (9,0)  {$\ttm{v}: 0, \{1\}$};
...
\if\riscvEd
\[
  \{ 0 \mapsto \key{t0}, \; 1 \mapsto \key{-24(fp)}, \; 2 \mapsto \key{-32(fp)} \}
\]
\fi
...
{\if\edition\pythonEd\pythonColor      
  ...
  \code{uncover\_live}, \code{build\_interference}, and
  \code{allocate\_registers}.
%  Run the \code{run-tests.py} script to
  Check whether the
  output programs produce the same result as the input programs.
  Inspect the generated {\ArchName} programs to make sure that some variables
...
The remaining step in the compilation to {\ArchName} is to ensure that
\ifxeightsix{the instructions have at most one argument that is a
  memory access.}
\ifriscv{arithmetic and logic instructions have no arguments that
  access memory.}
%
\if\architecture\xeightysixEd
...
problematic, but they can simply be deleted. In general, we wish to
delete all trivial moves whose source and destination are the
same location.
\fi
\if\architecture\riscvEd
...
instructions to satisfy the {\ArchName} calling conventions
...
\racket{add an entry to the \itm{info}
  of \code{{\ARCHNAME}Program} named \code{used\_callee}}
%
\python{add a field named \code{used\_callee} to the \code{{\ARCHNAME}Program} AST node}
%
...
saved and restored.
%
When calculating the amount to adjust the \SP{} in the prelude,
...
the amount $A$ to subtract from the \SP. Let $S$ be the number
...
variables (including the return address and the frame pointer). The
$\itm{align}$ function rounds a number up to the nearest 16 bytes.
\[
  \itm{A} = \itm{align}(8\itm{S} + 8\itm{C})
\if\architecture\xeightysixEd
- 8\itm{C}
\fi
\]
...
Figure~\ref{fig:running-example-x86} shows the {\ArchName} code generated for
...
this example to use just \ifxeightsix{two registers}\ifriscv{one
  register (with a different color numbering)}: \ifxeightsix{\code{rcx}}\ifriscv{\code{s0}} (color $0$)%
\ifxeightsix{ and {\code{rbx}} (color $1$)}. In the prelude\index{subject}{prelude} of the
\code{main} function, we push \ifxeightsix{\code{rbx}}\ifriscv{\code{s0}} onto the stack because it is
a callee-saved register and it was assigned to a variable by the
register allocator.
\if\architecture\xeightysixEd
We subtract \code{8} from the \SP{} at the
end of the prelude to reserve space for the one spilled variable.
After that subtraction, the \SP{} is aligned to 16 bytes.
\fi
...
\fi
\if\architecture\riscvEd
\python{Variables \code{v}, \code{x}, \code{y}, \code{tmp\_0},
  \code{tmp\_1}, and \code{tmp\_2}
  were assigned to \code{s0}, and variables \code{w} and \code{z}
  were assigned to \code{-24(fp)}.}
\fi
%
Variable \racket{\code{y}}\python{\code{z}} was spilled to the stack
location \ifxeightsix{\code{-16(\%rbp)}}\ifriscv{\code{-24(fp)}}.  Recall that the prelude saved the
callee-save register \ifxeightsix{\code{rbx}}\ifriscv{\code{s0}} onto the stack. The spilled variables
...
\ifxeightsix{\code{-16(\%rbp)}}\ifriscv{\code{-24(fp)}}.

In the conclusion\index{subject}{conclusion}, we undo the work that was
done in the prelude.
\if\architecture\xeightysixEd
We move the stack pointer up by \code{8} bytes
(the room for spilled variables), then pop the old values of
\ifxeightsix{\code{rbx}}\ifriscv{\code{s0}} and \FP{} (callee-saved registers), and finish with
\RET{} to return control to the operating system.
\fi
\if\architecture\riscvEd
We first restore the spilled callee-saved register \code{s0}, then the
return address and the frame pointer. Finally, we reset the stack
pointer and finish with
\RET{} to return control to the operating system.
\fi
...
\end{tcolorbox}
\end{minipage}
\caption{The {\ArchName} output from the running example
  (figure~\ref{fig:reg-eg}), limiting allocation to just \ifxeightsix{\code{rbx}
  and \code{rcx}}\ifriscv{\code{s0}}.}
...
opportunity for move biasing, and visually inspect the output {\ArchName}
...
making it straightforward to compile \code{if} statements to {\ArchName}.  The
...
statements to {\ArchName}.  The \LangCIf{} language also adds a \code{return}
...
comparison operations, and the \key{if} expression\python{ and
  statement}, we delve further into the {\ArchName}
...
subset of {\ArchName}, which includes instructions for logical operations,
...
As {\ArchName} does not provide direct support for Booleans, we take the usual
approach of encoding Booleans as integers, with \code{True} as $1$ and
\code{False} as $0$.

\if\architecture\xeightysixEd
Furthermore, {\ArchName} does not provide an instruction that directly
implements logical negation (\code{not} in \LangIf{} and \LangCIf{}).
However, the \IXOR{} instruction can be used to encode \code{not}.
The \IXOR{} instruction takes \ifxeightsix{two}\ifriscv{three} arguments, performs a pairwise
exclusive-or ($\mathrm{XOR}$) operation on each bit of its
\ifxeightsix{arguments}\ifriscv{second and third argument},
and writes the results into its \ifxeightsix{second}\ifriscv{first} argument.  Recall the following
...
the \code{not} operation can be implemented by \IXOR{} with $1$ as
the first argument.
\fi
\if\architecture\riscvEd
Fortunately, {\ArchName} provides an instruction that directly
implements logical negation (\code{not} in \LangIf{} and \LangCIf{}).
The instruction $\key{seqz}~d\key{,}~s$ (set if $=$ zero) sets $d$ to $1$ if $s=0$ and
sets $d$ to $0$ if $s\ne0$.
\fi
In the following schema, $\Arg$ is the translation of
$\Atm$ to {\ArchName}:
\[
...
\newcommand{\GrammarXIf}{
\begin{array}{lcl}
\itm{cc} & ::= & \key{eq} \MID \key{ne} \MID \key{lt} \MID \key{ge} \\
  \Instr &::=&
               % \key{xor}~\Arg\key{,}~\Arg\key{,}~\Arg \MID
               \key{s}cc\key{z} ~\Arg\key{,}~\Arg \MID
               \key{b}cc ~\Arg\key{,}~\Arg\key{,}~\itm{label} \\
\end{array}
}\fi
...
\begin{array}{lcl}
% \Arg &::=&  \gray{\IMM{\Int} \MID \REG{\Reg} \MID \DEREF{\Reg}{\Int}} \\
\itm{cc} & ::= & \skey{eq} \MID \skey{ne} \MID \skey{lt} \MID \skey{ge} \\
  \Instr &::=&
               % \TERINSTR{\scode{xor}}{\Arg}{\Arg}{\Arg} \MID
             \SETIF{\itm{cc}}{\Arg}{\Arg} \\
       &\MID& \python{\JMP{\itm{label}}} \MID  \JMPIF{\itm{cc}}{\Arg}{\Arg}{\itm{label}} 
\end{array}
}\fi
...
Next we consider the {\ArchName} instructions that are relevant for compiling
the comparison operations.
\if\architecture\xeightysixEd
The \key{cmpq} instruction compares its two
...
\fi
\if\architecture\riscvEd
Comparisons are either reified as boolean values in a register or they
are hard-wired into conditional branch instructions. A
branch instruction like $\key{beq}~r_1, r_2, \mathit{label}$ compares
the two registers $r_1$ and $r_2$ and jumps to \textit{label} if the
condition (i.e., $r_1 = r_2$) is true. Not all comparison operators
are directly available. For example, the comparison for less-than-or-equal
(\code{<=}) has to be implemented by 
using \key{ge} with the arguments swapped. There is also an unconditional
jump instruction $\key{j}~\mathit{label}$, which is needed for jumping
over the code for the else-branch of a conditional.
\fi
\if\architecture\xeightysixEd
...
instruction.  Like the abstract syntax for \code{set}, 
the abstract
...
\fi
\if\architecture\riscvEd
The abstract syntax for a conditional jump separates the instruction name from the
condition code. For example, \JMPIF{\QUOTE{\code{lt}}}{$a_1$}{$a_2$}{\QUOTE{\code{foo}}}
corresponds to \code{jlt $a_1$, $a_2$, foo}. The same holds for the
conditional set instruction that reifies a condition as a value in a
register: \SETIF{\QUOTE{\code{lt}}}{$d$}{$s$} corresponds to
\code{sltz $d$, $s$}.
\fi
...
\]
produces code with two {\ArchName} instructions (\INEG{} and \IADD)
instead of just one (\ISUB). Thus, we do not recommend
...
\fi
\if\architecture\riscvEd
Each comparison would first be translated into a boolean by
subtracting the arguments and testing the result.
In this case, the instruction \key{sltz} sets the destination register
if the source register is less than zero. Instruction \key{bnez}
branches if its argument is not equal to zero.
The generated code for the inner \key{if} in this
example would be as follows:
\begin{center}
\begin{minipage}{0.96\textwidth}
\begin{lstlisting}
  sub tmp_1, x, $1
  sltz tmp_2, tmp_1  # set tmp_2 if tmp_1<0
  bnez tmp_2, then_branch_1
  j else_branch_1
\end{lstlisting}
\end{minipage}
\end{center}
Notice that the first two instructions starting with \code{sub} are
redundant; instead we could generate the conditional jump right away.
\fi
...
%
At the level of {\ArchName} assembly, this is straightforward because we can
...
The \code{not} operation can be implemented in terms of \IXOR, as we
discussed at the beginning of this section.
For translating statements, we discuss some of the cases.
\if\architecture\xeightysixEd
Given an assignment, if
the left-hand-side variable is the same as the argument of \code{not},
then just the \IXOR{} instruction suffices.
\[
...
We consider the cases for equality comparisons.  Translating this
operation to {\ArchName} is slightly involved\ifxeightsix{ due to the unusual nature of the
...
be the translation of $\Atm_1$ to {\ArchName} and likewise for $\Arg_2$.\\
\begin{tabular}{lll}
\begin{minipage}{0.4\textwidth}
...
this one but use different condition codes for the \code{set} instruction.
...
\[
  \key{goto}\; \ell\racket{\key{;}} \quad \Rightarrow \quad
  \ifxeightsix{\key{jmp}\;\ell}
  \ifriscv{\key{j}\;\ell}
\]
%
An \key{if} statement becomes \ifxeightsix{a compare instruction followed by} a
...
\python{\ifxeightsix{Regarding the \key{return} statement, we recommend treating it
  as an assignment to the \RAX{} register followed by a jump to the
...
that are live before a jump should be the locations in
$L_{\mathsf{before}}$ at the target of the jump. So we recommend
...
$\JMPIF{\itm{cc}}{\Arg_1}{\Arg_2}{\itm{label}}$ to deal with.  Liveness analysis for
...
\ifxeightsix{Many of the}\ifriscv{The} new instructions in \LangXIfVar{} can be handled in the
...
\fi
\if\architecture\riscvEd
No particular issues arise with the new instructions. As usual,
immediate operands and memory operands have to be resolved to register
when translating from  \LangXIf{} to actual assembly code.
...
\end{tcolorbox}
  ...
  \caption{Example compilation of an \key{if} expression to \ArchName, showing
  the results of \code{explicate\_control},
  \code{select\_instructions}, and the final \ArchName{} assembly code.  }
...
{\if\edition\pythonEd\pythonColor
%
At first glance, the translation of \code{while} loops to \ArchName{} seems
...
only \RAX{} and \SP{}. So the live-before set
for \code{block8} is \code{\{\SP,sum\}}.
%
...
\begin{center}
\begin{lstlisting}
main_start: {}, block5: {}, block7: {}, block8: {}
\end{lstlisting}
\end{center}
...
\begin{center}
  \begin{lstlisting}
main_start: {}, block5: {i}, block7: {i, sum}, block8: {rsp, sum}
\end{lstlisting}
\end{center}
\fi}
{\if\architecture\riscvEd
\begin{center}
  \begin{lstlisting}
main_start: {}, block5: {i}, block7: {i, sum}, block8: {sp, sum}
\end{lstlisting}
\end{center}
\fi}

For the second round, the live-after for \code{main\_start} is the
current live-before for \code{block5}, which is \code{\{i\}}.  Therefore
the liveness analysis for \code{main\_start} computes the empty set. The
live-after for \code{block5} is the union of the live-before sets for
\code{block7} and \code{block8}, which is \code{\{i, \SP, sum\}}.
...
\begin{center}
  \begin{lstlisting}
main_start: {}, block5: {i, rsp, sum}, block7: {i, sum}, block8: {rsp, sum}
\end{lstlisting}
\end{center}
\fi}
{\if\architecture\riscvEd
\begin{center}
  \begin{lstlisting}
main_start: {}, block5: {i, sp, sum}, block7: {i, sum}, block8: {sp, sum}
\end{lstlisting}
\end{center}
\fi}
In the preceding iteration, only \code{block5} changed, so we can
limit our attention to \code{main\_start} and \code{block7}, the two
blocks that jump to \code{block5}.  As a result, the live-before sets
for \code{main\_start} and \code{block7} are updated to include
\SP, yielding the following approximation $m_3$:
{\if\architecture\xeightysixEd
\begin{center}
  \begin{lstlisting}
main_start: {rsp}, block5: {i,rsp,sum}, block7: {i,rsp,sum}, block8: {rsp,sum}
\end{lstlisting}
\end{center}
\fi}
{\if\architecture\riscvEd
\begin{center}
  \begin{lstlisting}
main_start: {sp}, block5: {i,sp,sum}, block7: {i,sp,sum}, block8: {sp,sum}
\end{lstlisting}
\end{center}
\fi}
Because \code{block7} changed, we analyze \code{block5} once more, but
its live-before set remains \code{\{i,\SP,sum\}}.  At this point
...
which corresponds to the direction of the \ArchName{} shifting instructions
\ifxeightsix{\key{salq} (shift left) and \key{sarq} (shift right)}%
\ifriscv{\key{sll} (shift left) and \key{srl} (shift right)}. Part of each tag
...
In this pass we generate {\ArchName} code for most of the new operations that
...
The tuple read and write forms translate into \IMOVE{}
...
are obtained by translating from \LangCVec{} to \ArchName.
%
The move of $\itm{tup}'$ to
register \ifxeightsix{\code{r11}}\ifriscv{\code{a1}} ensures that the offset expression
\ifxeightsix{\code{$8(n+1)$(\%r11)}}
\ifriscv{\code{$8(n+1)$(\%a1)}}
contains a register operand.  This requires
removing \ifxeightsix{\code{r11}}\ifriscv{\code{a1}} from consideration by the register allocator.

Why not use \RAX{} instead of \ifxeightsix{\code{r11}}\ifriscv{\code{a1}}? Suppose that we instead used
\RAX. Then the generated code for tuple assignment would be
{\if\architecture\xeightysixEd
\begin{lstlisting}
  movq |$\itm{tup}'$|, %rax
...
\code{patch\_instructions} would insert a move through \RAX{}
as follows:
{\if\architecture\xeightysixEd
\begin{lstlisting}
   movq |$\itm{tup}'$|, %rax
...
trying to use \RAX{} for two different values ($\itm{tup}'$ and
...
The \ArchName{} instructions \IAND{} (for bitwise-and) and \ISRL{}
(shift right) can accomplish this.
...
FromSpace, so we copy it into \ifxeightsix{\code{r11}}\ifriscv{\code{a1}} and then move it forward by
...
address in \ifxeightsix{\code{r11}}\ifriscv{\code{a1}} to the left-hand side. Refer to
...
\fi}
{\if\architecture\riscvEd
  The instruction \code{la} (load address) is a pseudo instruction
  that loads the address of a global variable in a register. A
  standard load instruction obtains the value of the global variable
  from that address. 
\begin{lstlisting}
   |$\itm{lhs}$| = allocate(|$\itm{len}$|, TupleType([|$\itm{type}, \ldots$])|);
   |$\Longrightarrow$|
   la a2, free_ptr        # a2 = address of free_ptr
   ld a1, 0(a2)           # a1 = value of free_ptr
   addi a0, a1, |$8(\itm{len}+1)$|
   sd 0(a2), a0           # save updated free_ptr
   li a2, |$\itm{tag}$|
   sd 0(a1), a2           # set tag at offset 0
   mv |$\itm{lhs}'$|, a1            # return pointer to tuple
\end{lstlisting}
  \fi}
%
...
allocated.  We use another dedicated register, \ifxeightsix{\code{r15}}\ifriscv{\code{s11}}, to store
the pointer to the top of the root stack. Therefore \ifxeightsix{\code{r15}}\ifriscv{\code{s11}} is not
...
start:
    ...
    la %a1, free_ptr
    ld tmp.517, 0(%a1)
    add tmp.518, tmp.517, $16
    la %a1, fromspace_end
    ld tmp.519, 0(%a1)
    blt tmp.518, tmp.519, block.529
    j block.530
...
block.528:
    ...
    la %a1, free_ptr
    ld tmp.520, 0(%a1)
    addi tmp.521, tmp.520, $16
    la %a1, fromspace_end
    ld tmp.522, 0(%a1)
    blt tmp.521, tmp.522, block.526
    j block.527
...
variables. We do so by incrementing the root stack pointer (\ROOTSP),
taking care that the root stack grows up instead of down.  For the
running example, there was just one spill, so we increment \ROOTSP{}
by 8 bytes. In the conclusion we subtract 8 bytes from \ROOTSP.
...
The \ArchName{} architecture provides a few features to support the
implementation of functions. We have already seen that there are
labels in \ArchName{} so that one can refer to the location of an instruction,
...
can obtain the address of a label by using the {\ILEA}
instruction. For example, the following puts the address of the
\code{inc} label into the \ifxeightsix{\code{rbx}}\ifriscv{\code{a1}} register:
\if\architecture\xeightysixEd
\begin{lstlisting}
    leaq inc(%rip), %rbx
\end{lstlisting}
...
In section~\ref{sec:x86} we used the {\ICALL} instruction to jump
...
a register; that is, we use \emph{indirect function calls}. 
\if\architecture\xeightysixEd
The {\ArchName} syntax for this is a {\ICALL} instruction that requires an asterisk
...
\fi
\if\architecture\riscvEd
The {\ArchName} syntax for this is a {\code{jalr}} (jump and link register) instruction.\index{subject}{indirect function call}
\begin{lstlisting}
    jalr a1
\end{lstlisting}
\fi
...
The \ifxeightsix{\code{callq}}\ifriscv{\code{jalr}} instruction provides partial support for implementing
functions: it 
\ifxeightsix{pushes the return address on the stack}
\ifriscv{saves the return address in register \code{ra}}
and it jumps to
the target. However, \ifxeightsix{\code{callq}}\ifriscv{\code{jalr}} does not handle
\begin{enumerate}
...
Regarding parameter passing, recall that the {\ArchName} calling
convention for Unix-based systems uses the following \ifxeightsix{six}\ifriscv{eight} registers to
...
\end{lstlisting}
\fi
If there are more than \ifxeightsix{six}\ifriscv{eight} arguments, then the calling convention
...
(section~\ref{sec:tail-call}), we arrange to always fit in \ifxeightsix{six}\ifriscv{eight}
arguments or less.
%
The return value of the function is stored in register \RAX.
...
stack pointer, register \SP, to the last data item in its
...
root stack pointer \ROOTSP{} up according to the number of variables
...
Recall that the \ifxeightsix{base}\ifriscv{frame} pointer, register \FP, is used as a
point of reference within a frame, so that each local variable can be
accessed at a fixed offset from the \ifxeightsix{base}\ifriscv{frame} pointer
(section~\ref{sec:x86}).
%
...
\begin{tcolorbox}[colback=white]
  \begin{tabular}{r|r|l|l} \hline
Caller View & Callee View & Contents       & Frame \\ \hline
-8(\key{fp})  & & return address & \multirow{5}{*}{Caller}\\
-16(\key{fp})  &  & old \key{fp} \\
-24(\key{fp}) &  & callee-saved $1$ \\
\ldots & & \ldots \\
$-8(j+2)$(\key{fp}) &  & callee-saved $j$ \\
$-8(j+3)$(\key{fp}) &  & local variable $1$ \\
\ldots & & \ldots \\
$-8(j+k+2)$(\key{fp}) &  & local variable $k$ &  $\leftarrow$ callee's \key{fp}\\
 %% & &  \\
%% $8n-8$\key{(\%rsp)} & $8n+8$(\key{\%rbp})& argument $n$ \\
%% & \ldots           & \ldots \\
%% 0\key{(\%rsp)} & 16(\key{\%rbp})  & argument $1$   & \\
\hline
& -8(\key{fp})   & return address & \multirow{5}{*}{Callee} \\
& -16(\key{fp})   & caller's \key{fp} \\
& -24(\key{fp}) & callee-saved $1$ \\
& \ldots & \ldots \\
& $-8(n+2)$(\key{fp})  & callee-saved $n$ \\
& $-8(n+3)$(\key{fp})  & local variable $1$ \\
&  \ldots          & \ldots \\
& $-8(n+m+2)$(\key{fp})   & local variable $m$  &  $\leftarrow$ callee's \key{sp}\\ \hline
\end{tabular}
\end{tcolorbox}
\fi
...
mentioned, for arguments beyond the \ifxeightsix{sixth}\ifriscv{eighth}, the convention is to use
...
the stack for passing more than \ifxeightsix{six}\ifriscv{eight} arguments
...
right one, and we should not use \ifxeightsix{\key{callq}}\ifriscv{\key{jalr}} to make the tail call
because that would overwrite the return address. Instead we simply use
the \ifxeightsix{\key{jmp}}\ifriscv{\key{jr}} instruction. As with the indirect function call, we write
an \emph{indirect jump}\index{subject}{indirect jump} with a register
\ifxeightsix{prefixed with an asterisk}\ifriscv{source}.  We recommend using \ifxeightsix{\RAX}\ifriscv{\code{t0}} to hold the
...
particular, we use {\ILEA} to convert the function name (a label
in \ArchName) to an address in a register.  Thus, we create a new pass that
...
Recall that we wish to limit the number of function parameters to \ifxeightsix{six}\ifriscv{eight}
...
transforms functions and function calls that involve more than \ifxeightsix{six}\ifriscv{eight}
arguments to pass the first \ifxeightsix{five}\ifriscv{seven} arguments as usual, but it packs the
rest of the arguments into a tuple and passes it as the \ifxeightsix{sixth}\ifriscv{eighth}
argument.\footnote{The implementation this pass can be postponed to
  ...
  \ifxeightsix{six}\ifriscv{eight} or fewer parameters.}

Each function definition with \ifxeightsix{seven}\ifriscv{nine} or more parameters is transformed as
follows:
{\if\edition\racketEd   
\begin{lstlisting}
...
|$\Rightarrow$|
  FunctionDef(|$f$|, [(|$x_1$|,|$T_1$|),|$\ldots$|,(|$x_\FIVE$|,|$T_\FIVE$|),(tup,TupleType([|$T_\SIX, \ldots, T_n$|]))],
         |$T_r$|, None, |$\itm{body}'$|, None)
\end{lstlisting}
\fi}
%
\noindent where the $\itm{body}$ is transformed into $\itm{body}'$ by
replacing the occurrences of each parameter $x_i$ where $i > \FIVE$ with
the $k$th element of the tuple, where $k = i - \SIX$.
%
{\if\edition\racketEd
\begin{lstlisting}
...
{\if\edition\pythonEd\pythonColor
\begin{lstlisting}
Call(|$e_0$|, [|$e_1,\ldots,e_\FIVE$|,Tuple([|$e_\SIX,\ldots,e_n$|])])
\end{lstlisting}
\fi}
\end{minipage}
\end{tabular}
...
ends up as an immediate argument of an {\ArchName} instruction. Function
...
needs to be converted to an address using the {\ILEA}
...
\newcommand{\GrammarXIndCall}{
\begin{array}{lcl}
\Instr &::=&  \key{jalr}\;\Arg \MID \key{tailjmp}\;\Arg 
     \MID \key{la}\;\key{\%}\Reg\key{,}\; \Arg \\
\Block &::= & \Instr^{+} \\
\Def &::= & \code{.globl}\,\code{.align 8}\,\itm{label}\; (\itm{label}\key{:}\, \Block)^{*} 
\end{array}
}\fi
...
\begin{array}{lcl}
  ...
  \Instr &::=& \ldots \MID \INDCALLQ{\Arg}{\itm{int}}
    \MID \TAILJMP{\Arg}{\itm{int}}\\
    &\MID& \BININSTR{\scode{la}}{\REG{\Reg}}{\Arg}\\
...
load-\ifxeightsix{effective-}address instruction as follows, where $\itm{lhs}'$ is
...
$\RETURN{Arg}$ can be compiled to an assignment to
\ifxeightsix{\code{rax}}\ifriscv{\code{a0}} followed 
...
{\IMOVE} instruction. This happens in the example translation given
in figure~\ref{fig:add-fun}, in
...
\ifxeightsix{\code{rsi}}\ifriscv{\code{a1}}, and that will prevent the mapping of $x_1$ to \ifxeightsix{\code{rsi}}\ifriscv{\code{a1}},
which is good because otherwise the first {\IMOVE} would overwrite
the argument in \ifxeightsix{\code{rsi}}\ifriscv{\code{a1}} that is needed for $x_2$.
...
are moved to the argument-passing registers.  The function
...
call. The return value from the function is stored in {\RAX}, so
...
The \code{IndirectCall\ifxeightsix{q}} instruction should be treated like
{\ICALL} regarding its written locations $W$, in that they should
include all the caller-saved registers. This treatment forces
variables that are live across a function call to be assigned to
callee-saved registers or to be spilled to the stack.

Regarding the set of read locations $R$, the arity fields of
\code{TailJmp} and \code{IndirectCall\ifxeightsix{q}} determine how many of the
argument-passing registers should be considered as read by those
instructions. Also, the target field of \code{TailJmp} and
\code{IndirectCall\ifxeightsix{q}} should be included in the set of read locations
$R$.
...
In section~\ref{sec:reg-alloc-gc} we discussed the need to
...
In \code{patch\_instructions}, you should deal with the {\ArchName}
idiosyncrasy that the destination argument of {\ILEA} must be a
register. Additionally, you should ensure that the argument of
\code{TailJmp} is \ifxeightsix{\RAX}\ifriscv{\code{t0}}, our reserved register---because we
...
\code{TailJmp} would simply be
\ifxeightsix{\code{jmp *$\itm{arg}$}}\ifriscv{\code{jr $\itm{arg}$}}.  However,
...
the conclusion of a function, except that the \RET{} is replaced with
...
%%   example.)
  \if\architecture\riscvEd
\item Push the return address \code{ra} to the stack.
  \fi
\item Push \FP{} to the stack and set {\FP} to current stack
  pointer\ifriscv{$+16$}.
\item Push to the stack all the callee-saved registers that were
  used for register allocation.
\item Move the stack pointer {\SP} down to make room for the
  regular spills (aligned to 16 bytes).
\item Move the root stack pointer {\ROOTSP} up by the size of the
...
\ROOTSP. This initialization should happen before step
\ref{root-stack-init}, which depends on \ROOTSP.
...
\item Restore \FP by popping it from the stack.
\item Return to the caller with the \RET{} instruction.
\end{enumerate}
...
\end{tabular}
  ...
  \caption{Example compilation of a simple function to \ArchName.}
\label{fig:add-fun}
\end{figure}
...
Instead, we put a list of type variables (such as \code{T}) between
the name of the function and its parameter list.
%
\fi}
...
\begin{array}{lcl}
   \Def &::=& \CDEF{\Var[\Var\ldots]}{\Var \key{:} \Type\key{,} \ldots}{\Type}{\Stmt^{+}}  \\
...
{\if\edition\pythonEd\pythonColor
\begin{lstlisting}
def apply_twice[U](f : All[[U], Callable[[U],U]]) -> int:
  if f(True):
    return f(42)
  else:
    return f(777)
...
parameters in both argument types to fresh type parameters.
